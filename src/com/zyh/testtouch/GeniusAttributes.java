...
public class GeniusAttributes extends Attributes {
    ...
    public static final String[] DEFAULT_FONT_WEIGHT = new String[] { "bold", "extrabold", "extralight",
            "light", "regular" };
    ...
    public void initHasOwnAttrs(Context context, AttributeSet attrs) {
        ...
        if (textColorAttribute == null) {
            ...
            try {
                TypedArray styleTextColorTypedArray = context
                        .obtainStyledAttributes(styleId, attributesArray);
                // color might have values from the entire integer range, so to
                // find out if there is any color set,
                // checking if default value is returned is not enough. Thus we
                // get color with two different
                // default values - if returned value is the same, it means
                // color is set
    ...
    public void initCornerRadius(TypedArray a, int indexRadius, int indexRadiiA, int indexRadiiB,
            int indexRadiiC, int indexRadiiD) {
        // Set Radius
