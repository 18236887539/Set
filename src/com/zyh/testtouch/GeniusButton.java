...
public class GeniusButton extends Button implements Attributes.AttributeChangeListener {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            mAttributes.setFontFamily(GeniusAttributes.DEFAULT_FONT_FAMILY[a.getInt(
                    R.styleable.GeniusButton_g_fontFamily, 0)]);
            mAttributes.setFontWeight(GeniusAttributes.DEFAULT_FONT_WEIGHT[a.getInt(
                    R.styleable.GeniusButton_g_fontWeight, 3)]);
            mAttributes.setFontExtension(a.getString(R.styleable.GeniusButton_g_fontExtension));
            mAttributes.setTextAppearance(a.getInt(R.styleable.GeniusButton_g_textAppearance,
                    GeniusAttributes.DEFAULT_TEXT_APPEARANCE));
            mAttributes.setBorderWidth(a.getDimensionPixelSize(R.styleable.GeniusButton_g_borderWidth,
                    GeniusAttributes.DEFAULT_BORDER_WIDTH));
            ...
            mAttributes.setBottom(a.getDimensionPixelSize(R.styleable.GeniusButton_g_blockButtonEffectHeight,
                    mAttributes.getBottom()));
            ...
            mAttributes.setDelayClick(a.getBoolean(R.styleable.GeniusButton_g_delayClick,
                    mAttributes.isDelayClick()));
        ...
        if (!mAttributes.isHasOwnTextColor()) {
            if (mAttributes.getTextAppearance() == 1)
                setTextColor(mAttributes.getColor(0));
            else if (mAttributes.getTextAppearance() == 2)
                setTextColor(mAttributes.getColor(3));
            else
    ...
    private Drawable[] initBackground() {
        ...
        ShapeDrawable normalFront = new ShapeDrawable(new RoundRectShape(mAttributes.getOuterRadii(), null,
                null));
        normalFront.getPaint().setColor(mAttributes.getColor(2));

        if (mAttributes.getBottom() > 0) {
            ShapeDrawable normalBack = new ShapeDrawable(new RoundRectShape(mAttributes.getOuterRadii(),
                    null, null));
        ...
        ShapeDrawable pressedFront = new ShapeDrawable(new RoundRectShape(mAttributes.getOuterRadii(), null,
                null));
        pressedFront.getPaint().setColor(mAttributes.getColor(1));

        if (mAttributes.getBottom() > 0) {
            ShapeDrawable pressedBack = new ShapeDrawable(new RoundRectShape(mAttributes.getOuterRadii(),
                    null, null));
        ...
        ShapeDrawable disabledFront = new ShapeDrawable(new RoundRectShape(mAttributes.getOuterRadii(), null,
                null));
        disabledFront.getPaint().setColor(mAttributes.getColor(3));
        disabledFront.getPaint().setAlpha(0xA0);

        if (mAttributes.getBottom() > 0) {
            ShapeDrawable disabledBack = new ShapeDrawable(new RoundRectShape(mAttributes.getOuterRadii(),
                    null, null));
